    ...
    async (lat, lng) => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.REACT_APP_API_URL || (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:3000' : 'https://medi-vault-zsg1.onrender.com')}/api/medical-facilities?lat=${lat}&lng=${lng}&type=hospital,pharmacy`
        );
